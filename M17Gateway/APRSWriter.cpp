...
void CAPRSWriter::sendIdFrameFixed()
{
	...
	if (m_txFrequency != 0U) {
		...
		::sprintf(desc, "MMDVM Voice%.6LfMHz %c%.4lfMHz%s%s",
	...
	if (pos == std::string::npos)
        ...
        char output[1500U];
        ::sprintf(output, "%s>APDG03,TCPIP*,qAC,%s:!%s%cW%s%cn/A=%06.0f%s %s\r\n%s>APDG03:>Pwrd. by W0CHP-PiStar-Dash (https://wpsd.w0chp.net)\r\n",                                               
...
void CAPRSWriter::sendIdFrameMobile()
{
	...
	if (pos == std::string::npos)
        ...
        char output[1500U];
        ::sprintf(output, "%s>APDG03,TCPIP*,qAC,%s:!%s%cW%s%cn\r\n%s>APDG03:>Pwrd. by W0CHP-PiStar-Dash (https://wpsd.w0chp.net)\r\n",
                m_callsign.c_str(), server.c_str(),
                lat, (m_latitude < 0.0F)  ? 'S' : 'N',
                lon, (m_longitude < 0.0F) ? 'W' : 'E',
                m_callsign.c_str());
