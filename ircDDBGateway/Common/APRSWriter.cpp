...
void CAPRSWriter::sendIdFramesFixed()
{
	...
	for (CEntry_t::iterator it = m_array.begin(); it != m_array.end(); ++it) {
		...
		wxString output;
		output.Printf(wxT("%s-S>APDG01,TCPIP*,qAC,%s-GS:;%-7s%-2s*%02d%02d%02dz%s%c/%s%c&RNG%04.0lf/A=%06.0lf %s %s\r\n%s-S>APDG01:>Powered by W0CHP-PiStar-Dash (https://wpsd.w0chp.net)\r\n"),
		...
		if (entry->getBand().Len() == 1U) {
			output.Printf(wxT("%s-%s>APDG02,TCPIP*,qAC,%s-%sS:!%s%c/%s%c&RNG%04.0lf/A=%06.0lf %s %s\r\n%s-%s>APDG02:>Powered by W0CHP-PiStar-Dash (https://wpsd.w0chp.net)\r\n"),
...
void CAPRSWriter::sendIdFramesMobile()
{
	...
	for (CEntry_t::iterator it = m_array.begin(); it != m_array.end(); ++it) {
		...
		wxString output1;
		output1.Printf(wxT("%s-S>APDG01,TCPIP*,qAC,%s-GS:;%-7s%-2s*%02d%02d%02dz%s%c/%s%c&/A=%06.0lf"),
			m_gateway.c_str(), m_gateway.c_str(), entry->getCallsign().c_str(), entry->getBand().c_str(),
			tm->tm_mday, tm->tm_hour, tm->tm_min,
		...
		if (entry->getBand().Len() == 1U) {
			if (altitudeSet)
				output1.Printf(wxT("%s-%s>APDG02,TCPIP*,qAC,%s-%sS:!%s%c/%s%c&/A=%06.0lf"),
					entry->getCallsign().c_str(), entry->getBand().c_str(), entry->getCallsign().c_str(), entry->getBand().c_str(),
					lat.c_str(), (rawLatitude < 0.0)  ? wxT('S') : wxT('N'),
					lon.c_str(), (rawLongitude < 0.0) ? wxT('W') : wxT('E'),
					rawAltitude * 3.28);
			else
				output1.Printf(wxT("%s-%s>APDG02,TCPIP*,qAC,%s-%sS:!%s%c/%s%c&"),
					entry->getCallsign().c_str(), entry->getBand().c_str(), entry->getCallsign().c_str(), entry->getBand().c_str(),
					lat.c_str(), (rawLatitude < 0.0)  ? wxT('S') : wxT('N'),
					lon.c_str(), (rawLongitude < 0.0) ? wxT('W') : wxT('E'));
